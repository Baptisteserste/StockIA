...
/**
 * Script de test pour diagnostiquer les réponses de l'Agent Premium
 * N'utilise PAS la DB, juste des données mockées
 */

async function testPremiumAgent() {
    console.log('🧪 Testing Premium Agent (ISOLATED - No DB writes)');
    ...
    // Model ID - Test Gemini 2.5 Flash
    ...
    console.log('\n🤖 Calling Premium Agent...\n');

    try {
        const decision = await premiumAgent.decide(mockSnapshot, mockPortfolio, modelId);

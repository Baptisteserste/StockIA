...
import { TrendingUp, TrendingDown, Settings, Trophy, History, Lock } from 'lucide-react';
...
export default function SimulationPage() {
  const { userId } = useAuth(); // Pour sécuriser les appels API
  ...
  // Fetch models au mount (Seulement si connecté)
  useEffect(() => {
    if (!userId) return;
    ...
    fetch('/api/openrouter/models')
        .then(r => {
          if (!r.ok) throw new Error("Erreur chargement modèles");
          return r.json();
        })
        .then(data => {
          setModels(data);
  ...
  // Fetch simulation status au mount et polling (Seulement si connecté)
  useEffect(() => {
    if (!userId) return;
          ...
          <SignedOut>
            <div className="max-w-xl mx-auto text-center py-20 bg-slate-800/50 rounded-2xl border border-slate-700 mt-10">
              ...
              <p className="text-slate-400 mb-8 px-6">
                Connectez-vous pour configurer et lancer vos simulations de trading avec nos agents IA.
              </p>
